...
function adm_edit_product($site_id, $prod_id)
{
...
function adm_assign_categories($site_id, $prod_id)
{
   ...
   // TODO: This is not very robust because it depends on the CategoryOrder
   // field to get the hierarchy straight. It should be re-thought.
...
//      echo "<pre>"; print_r($source); echo "</pre>";
      ...
      for($i=0; $i<count($source); $i++) {
         if ($source[$i]['checked'] == "YES") {
            if (empty($values[$source[$i]['CategoryID']])) {
...
//  In the future, I'd like to get this data based on the site table and
//  the preferences set in the Admin section.
// ------------------------------------------------------------------------
