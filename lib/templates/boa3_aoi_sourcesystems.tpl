...
<!--
var toggle_sourcesystems = false;
function doallsourcesystems()
{
  toggle_sourcesystems = !toggle_sourcesystems;
  for(i = 0; i < document._form["sourcesystem[]"].length; i++)
    document._form["sourcesystem[]"][i].checked = toggle_sourcesystems;
}
...
function dosave()
{
   ok = false;
   for (i = 0; i < document._form["sourcesystem[]"].length; i++) {
      if (document._form["sourcesystem[]"][i].checked) {
         ok = true;
      }
   }
...
<form name="_form" action="aoi_sourcesystems.php" method="post">
...
<div class="datarow1">
   ...
   <input type="checkbox" id="usertype" name="usertype[]" value="SuperUser"{if $isSuperUser == 1} checked="checked"{/if} onchange="detectChange(1000);" title="give this user access to all source systems" /><label for="usertype" title="give this user access to all source systems">Super User</label><br />

</div> {* datarow1 *}

<div class="datarow1">

<p>Source Systems (select all that apply):</p>
<input type="button" value="All/none" onclick="doallsourcesystems()" /><br />

<table width="100%" border="0" cellspacing="0" cellpadding="4">
<tr>
<td width="34%" valign="top">
{section name="reg" loop=$sourcesystems}
   {if (($smarty.section.reg.index % $col_length) == 0)  && !$smarty.section.reg.first}
      </td><td width="33%" valign="top">
   {/if}
   <span class="datarowSm"><input type="checkbox" id="sourcesystem{$smarty.section.reg.index}" name="sourcesystem[]" value="{$sourcesystems[reg].SOURCESYSTEMKEY}"{if $sourcesystems[reg].selected == 1} checked="checked"{/if} onchange="detectChange({$smarty.section.reg.index});" /><label for="sourcesystem{$smarty.section.reg.index}">{$sourcesystems[reg].DESCRIPTION}</label></span><br />
{/section}
</td>
</tr>
</table>
