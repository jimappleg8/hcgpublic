...
{section name="reg" loop=$regions }
   ...
   {if ($regions[reg].DWSPERSONCODE == -1) }
      ...
      <tr class="datarow1">
      <td>&nbsp;</td>
      <td>{if $regions[reg].highlight == 0}<font color="#999999">{else}<font color="#FF0000">{/if}
...
{section name="chan" loop=$channels }
   ...
   {section name="div" loop=$divisions }
      ...
      {if ($divisions[div].DWDIVISIONCODE > $channels[chan].DWCHANNELCODE) &&
         ...
         <tr class="datarow1">
         <td>{if $divisions[div].highlight == 0}<font color="#999999">{else}<font color="#FF0000">{/if}
         {$divisions[div].DWDIVISIONCODE}</font></td>
         <td colspan="2">{if $divisions[div].highlight == 0}<font color="#999999">{else}<font color="#FF0000">{/if}
