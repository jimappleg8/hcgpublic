...
function dosave()
{
  if(!document._form.user_name.value.length)
  {
    alert("Please enter the new user's name.");
    return;
  }

  if(!document._form.location.selectedIndex)
  {
    alert("Please choose this user's location.");
    return;
  }
...
<span class="pageHd">Business Objects New User Request</span>

{if $what == "save"}

<p class="notice">Thank you!
<br />
<br />Your request has been sent to the Help Desk. Please watch your e-mail for your ticket number and confirmation for this request.
<br />
<br />Requests take 1-3 days to complete; you will be notified via e-mail when the new user account is activated.
<br />
<br />If this request is urgent, please contact the Help Desk at 303-581-1310 after submitting the request, and discuss the situation with an Information Technology representative.</p>
...
<form name="_form" action="user_new.php" method="post">
...
<tr class="datarow1">
  <td>
    <input type="button" value="Another"
      onclick="document.location='user_new.php'" />
  </td>
</tr>
...
<tr class="datarow1">
  <td colspan="3">
    User's full name:<br />
    <input type="textfield" name="user_name" size="30" />
  </td>
</tr>
...
{section name="locale" loop=$locations}
   <option value="{$locations[locale]}">{$locations[locale]}</option>
{/section}
</select>
      ...
      {if ($divisions[div].DWDIVISIONCODE > $channels[chan].DWCHANNELCODE) &&
         ...
         {section name="reg" loop=$regions }
            ...
            {if ($regions[reg].DWSPERSONCODE > $divisions[div].DWDIVISIONCODE) &&
               ...
               <tr class="datarow1">
               <td><input type="checkbox" name="region[]" value="{$regions[reg].DWSPERSONCODE}" /></td>
               <td>{$regions[reg].DWSPERSONCODE}</td>
               <td>{$regions[reg].DWSPERSONDESC}</td></tr>
