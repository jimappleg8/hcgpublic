            ...
            if ($prefix == 'chname' && is_numeric($id) && !empty($value) && ($_POST['origname_'.$id] != $value) && isset($_POST['submit_changeusers'])) {	    
                if (liveuser_checkEntity('user', $value) === false) {
                    if ($liveuserAuthAdmin->updateUser($id, $value)) {	    
                        echo '<p>User '.$value.' was successfully updated.</p>';
                    } else {
                        echo '<p>Update of user '.$value.' failed.</p>';
                    }
                } else {
                    echo '<p>Another user with the name '.$value.' already exists in the database. No change has been made.</p>';
                }	
            }
