...
{
    global $liveuser, $liveuserDB, $liveuserPermRings;

    // editing a new page: empty data array, and not a plugin page (implies new page)
    if (!empty($data['id']) && empty($data['content']) && !array_key_exists($page_name, $GLOBALS['ewiki_plugins']['page'])) {
        $page_name = '[NewPage]';
    }
...
{   
    ...
    // set default rights for new pages if not authenticated or form data is unavailable
