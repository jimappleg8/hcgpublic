<?php
 ...
 function ewiki_page_gnu_diff($id, $data) {
...
#   $OPTIONS = " --side-by-side ";
    ...
    (EWIKI_PRINT_TITLE)
    && ($o = "<h3>Differences between version $new_ver and $old_ver of »{$id}«</h3>\n");

    #-- create temporary files from wikipages
    $file0 = tempnam("/tmp", "ewiki.diff.gnu.");
    $f = fopen($file0, "w");
    fwrite($f, $data0["content"]);
    fclose($f);
    $file1 = tempnam("/tmp", "ewiki.diff.gnu.");
    $f = fopen($file1, "w");
