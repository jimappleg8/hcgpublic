...
<?php
  ...
  if (empty($_REQUEST["set"])) {
     ...
     echo '<TABLE BORDER="0" CELLSPACING="3" CELLPADDING="2" WIDTH="600">' . "\n";
     ...
     while ($row = $result->get()) {
        ...
        echo $id . '</A><small>' .  (".".$row["version"]) . '</small></TD>';

        echo '<TD BGCOLOR="#EEEEEE"><small>';
        foreach ($FD as $n=>$str) {
           echo '<INPUT TYPE="checkbox" NAME="set['. $id
                . '][' . $n . ']" VALUE="1" '
     ...
     foreach($_REQUEST["set"] as $page=>$fa) {
        ...
        echo "· {$page} (flags = {$flags} <small>[{$fstr}]</small> )";

        $data = ewiki_database("GET", array("id" => $page));

        if ($data["flags"] != $flags) {
           $data["flags"] = $flags;
           $data["author"] = "ewiki-tools";
           $data["version"]++;
